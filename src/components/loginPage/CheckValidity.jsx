...
const CheckValidity = ({ setStep, phoneNumber }) => {
  ...
  const [classes, setClasses] = useState(
    "outline-none bg-neutral-200 lg:bg-white border-b-2 lg:border border-teal-400 focus:border-teal-400 caret-teal-400 rounded-lg text-center w-full p-3 mt-4"
  );
  ...
  const inputStyleHandler = () => {
    otpValidationError
      ? setClasses(
          "outline-none bg-neutral-200 lg:bg-white border-b-2 lg:border border-red-400 focus:border-red-400 caret-red-400 rounded-lg text-center w-full p-3 mt-4"
        )
      : inputValue == otpCode
      ? setClasses(
          "outline-none bg-neutral-200 lg:bg-white border-b-2 lg:border border-teal-400 focus:border-teal-400 caret-teal-400 rounded-lg text-center w-full p-3 mt-4"
  ...
  return (
    <>
      ...
      <div className="w-full flex flex-col justify-center items-center fixed inset-0 z-10">
        <div
          className="w-full lg:border-2 border-neutral-200 rounded-xl p-6"
          style={{ maxWidth: 400 }}
        >
          {isDesktop && (
          ...
          <div className="flex justify-center text-xs py-4">
            {minute === 0 && second === 0 ? (
              ...
              <p>
                {" "}
                {second} : {minute} مانده تا دریافت مجدد کد{" "}
              </p>
            )}
          </div>
          <button
