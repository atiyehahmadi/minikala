...
const ProductsGridMobile = () => {
  ...
  const { data: productsInfo } = useFetch({
    url: `http://localhost:8000/products?${id ? `category=${id}` : ""}${
      sortType ? `&_sort=${sortType}&_order=DESC` : ""
    }`,
  ...
  return (
    <>
      ...
      <div className="fixed -bottom-5 left-0 bg-white border border-neutral-300 rounded-t-xl px-4 pt-4 z-50 w-full">
        {showSortMenu && (
          <>
            <div
              className="flex gap-2 font-bold text-md items-center cursor-pointer overflow-y-scroll py-2"
            ...
            {sortTypeList.map((item, index) => (
              <div
                key={index}
                className="flex justify-between items-center border-b border-neutral-200 text-neutral-500 cursor-pointer p-4 last:pb-10"
      ...
      <div className="grid md:grid-cols-3 sm:grid-cols-2 grid-flow-row">
        {productsInfo?.map((item, index) => (
          <ProductCard key={index} dataList={item} />
        ))}
      </div>
      ...
      <div
        className={`${
          showSortMenu ? "bg-black opacity-40 fixed inset-0 z-10" : ""
        }`}
      ></div>
    </>
  );
