    ...
    <>
      <div className="flex gap-4 py-2">
        <div className="flex gap-2">
          ...
          <p className="text-sm text-neutral-800 font-semibold">مرتب‌سازی:</p>
        </div>
        {sortTypeList.map((item, index) => (
          <div key={index} onClick={() => sortHandler(item.sortBy, item.title)}>
            <p
              className={`text-sm text-neutral-500 cursor-pointer ${
                item.title === sortName ? "text-red-500" : ""
              }`}
            >
              {item.title}
            </p>
