...
const FilterMenu = ({ closeFilterModal, classes }) => {
  ...
  const { data: filterItem } = useFetch({
    url: "http://localhost:8000/filterTitle",
  });

  return (
    <div
      className={`fixed z-10 right-0  top-0 lg:border-neutral-200 bg-white lg:rounded-lg pb-10 px-4 w-full overflow-y-scroll ${classes}`}
    >
      ...
      <div>
        {filterItem?.map((item, index) => (
          <FilterMenuItem key={index} dataList={item} idBtn={index} />
        ))}
      </div>
    </div>
  );
