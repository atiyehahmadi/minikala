...
const FilterSubMenuItem = ({ dataList }) => {
  ...
  useEffect(() => {
    const filters = dataList.data.map((item, index) => (
  ...
  return (
    <div className="mr-4">
      {dataList.data.map((item, index) => (
        <div
          key={index}
          className="flex gap-4 items-end border-b border-neutral-200 cursor-pointer py-4 mr-4"
          onClick={() => showSubMenuHandler(item.id)}
        >
          <ArrowBottomIcon className="w-3 h-3 text-neutral-600" />
