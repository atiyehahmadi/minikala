        ...
        <>
          <div className="py-2 my-2 overflow-x-auto">
            <div className="flex gap-1 items-end ">
              <WatchIcon className="w-5 h-5 text-neutral-600" />
              <p className="text-md font-bold text-neutral-700">
                آخرین جستجوی های شما
              </p>
              <TrashIcon
                className="mr-auto w-5 h-5 text-neutral-600 cursor-pointer"
                click={() => setShowLastsearches(false)}
              />
            </div>
            <div className="flex gap-1 py-2">
              {[...new Set(lastSearchItems)].map((item, index) => (
                <SearchItemBox text={item} key={index} />
              ))}
            </div>
          </div>
        </>
