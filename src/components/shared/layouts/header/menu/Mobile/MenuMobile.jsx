...
const MenuMobile = () => {
  ...
  const { data: menuItem } = useFetch({
    url: "http://localhost:8000/megaMenuList",
  });

  return (
    <div
      className="fixed top-0 bg-white border border-neutral-200 overflow-y-scroll w-72 h-full z-30"
      ref={modalRef}
    >
      <div className="pr-2">
        <img src={LogoPersian} alt="" className="w-16 h-16" />
        <div className="border-y border-y-neutral-300">
        ...
        <div className="relative">
          {menuItem?.map((item, index) => (
            <MenuMobileItem key={index} dataList={item} />
          ))}
        </div>
      </div>
    </div>
