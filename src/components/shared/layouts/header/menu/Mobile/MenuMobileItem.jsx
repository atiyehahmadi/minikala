...
const MenuMobileItem = ({ dataList }) => {
  ...
  const { data: subMenuItem } = useFetch({
    url: `http://localhost:8000/megaMenuList?id=${subMenuId}`,
  ...
  return (
    <>
      <div
        className="flex items-center text-xs cursor-pointer py-4 px-2 active:text-red-500"
        onClick={showSubMenuHandler}
      >
        <img src={dataList.icon} alt="" className="w-5 h-5" />
        <p className="text-sm px-2">{dataList.title}</p>
        {showSubMenu ? (
          <ArrowTopIcon className="w-4 h-4 mr-auto ml-4 font-bold text-red-600" />
        ) : (
      ...
      {showSubMenu &&
        subMenuItem?.map((item, index) => (
          <SubMenuMobileItem key={index} dataList={item} />
        ))}
    </>
  );
};
